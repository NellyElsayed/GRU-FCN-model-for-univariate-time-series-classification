...
def generate_model_saqqara():
    ip = Input(shape=(1, MAX_SEQUENCE_LENGTH))

    x= BatchNormalization()(ip)
    x = GaussianDropout(0.8)(ip)
    x =  GRU(15, return_sequences=True,kernel_initializer='TruncatedNormal')(x)
#    x = GaussianDropout(0.2)(x)
    x =  GRU(10, return_sequences=True,kernel_initializer='TruncatedNormal')(x)
 #   x = GaussianDropout(0.2)(x)
    x =  GRU(5, return_sequences=False,kernel_initializer='TruncatedNormal')(x)
 ...
 #   x = AlphaDropout(0.5)(x)
    ...
    y = Permute((2, 1))(ip)
    y = Conv1D(64,10, padding='same', kernel_initializer='he_uniform')(y)
    y = BatchNormalization()(y)
    y = Activation('tanh')(y)
  #  y = squeeze_excite_block(y)
    
    y = Conv1D(128, 20, padding='same', kernel_initializer='he_uniform')(y)
    y = BatchNormalization()(y)
    y = Activation('tanh')(y)
    #y = squeeze_excite_block(y)

    y = Conv1D(256, 30, padding='same', kernel_initializer='he_uniform')(y)
    y = BatchNormalization()(y)
    y = Activation('tanh')(y)
    
 #   x = GaussianDropout(0.2)(x)
  #  y= Flatten()(y)
    y = GlobalAveragePooling1D()(y)#GlobalAveragePooling1D()(y)
...
if __name__ == "__main__":
    ...
    history = train_model(model, DATASET_INDEX, dataset_prefix='cinc_ecg_torso', epochs=500, batch_size=128)
    summary = model.summary()
    accuracy, loss, f_score= evaluate_model(model, DATASET_INDEX, dataset_prefix='cinc_ecg_torso', batch_size=128)
